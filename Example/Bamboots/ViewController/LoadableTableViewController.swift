...
extension LoadableTableViewController {
    fileprivate func initTableView() {
        ...
        tableHeaderView?.frame = CGRect(
            x: 0, y: 0, width: UIScreen.main.bounds.width, height: UIScreen.main.bounds.width
        )
...
class LoadableTableViewController: UITableViewController, Requestable {
    ...
    @IBAction func load(_ sender: AnyObject) {
        let load = LoadConfig(
            ...
            inset: UIEdgeInsets(
                top: UIScreen.main.bounds.width - self.tableView.contentOffset.y > 0 ?
                    UIScreen.main.bounds.width - self.tableView.contentOffset.y : 0,
                left: 0,
                bottom: 0,
                right: 0
            )
